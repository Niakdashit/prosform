...
import { ArrowLeft, Download, Trophy, Calendar, Users, Gift, Filter } from "lucide-react";
  ...
  const handleExportAll = async () => {
    try {
      const data = allParticipations.map(p => ({
        ...
        referrer: p.referrer,
        participation_data: { prize: p.prize_won?.name || '', ip: p.ip_address }
      }));
      const csvContent = ExportService.convertToCSV(data);
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Données contact</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {winners.map((winner) => (
                    <TableRow key={winner.id}>
                      ...
                      <TableCell className="text-xs">
                        {winner.participation_data ? JSON.stringify(winner.participation_data).substring(0, 50) + '...' : '-'}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
