...
import { 
  jackpotTemplates, 
  jackpotTemplateCategories, 
  JackpotTemplate, 
  JackpotTemplateCategory,
  searchJackpotTemplates,
  getJackpotTemplatesByCategory 
...
interface JackpotTemplateLibraryProps {
  onSelectTemplate: (config: Partial<JackpotConfig>, templateMeta: {
    colorPalette: JackpotTemplate['colorPalette'];
    typography: JackpotTemplate['typography'];
...
export const JackpotTemplateLibrary = ({ onSelectTemplate }: JackpotTemplateLibraryProps) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedCategory, setSelectedCategory] = useState<JackpotTemplateCategory | "all">("all");
  const [previewTemplate, setPreviewTemplate] = useState<JackpotTemplate | null>(null);
  const [sortBy, setSortBy] = useState<"popular" | "new">("popular");

  // Filter and sort templates
  const filteredTemplates = useMemo(() => {
    let templates = searchQuery 
      ? searchJackpotTemplates(searchQuery)
      : selectedCategory === "all" 
        ? jackpotTemplates 
        : getJackpotTemplatesByCategory(selectedCategory);
  ...
  const handleUseTemplate = (template: JackpotTemplate) => {
          ...
          <ScrollArea className="h-full">
            <div className="p-4">
              ...
              <div className="space-y-1">
                ...
                {jackpotTemplateCategories.map((category) => {
                  const count = getJackpotTemplatesByCategory(category.id).length;
                  return (
                    <button
                      key={category.id}
          ...
          <div className="p-6">
            ...
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-muted-foreground">
                ...
                {selectedCategory !== "all" && !searchQuery && ` dans ${jackpotTemplateCategories.find(c => c.id === selectedCategory)?.name}`}
              </p>
            </div>
