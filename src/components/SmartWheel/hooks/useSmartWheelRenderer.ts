...
export const useSmartWheelRenderer = ({
  ...
  useEffect(() => {
    const checkAssets = () => {
      ...
      if (allReady && !assetsReadyNotifiedRef.current) {
        console.log('✅ All assets ready, allowing render with no extra delay');
        // Pas de délai supplémentaire : affichage immédiat dès que les assets sont prêts
        setAssetsReady(true);
  ...
  useEffect(() => {
    ...
    // Désactivées pour les styles premium (or/argent/rainbow) et classique afin d'avoir un rendu plus propre
    const bulbsDisabledForStyle =
      borderStyle === 'classic' ||
      borderStyle === 'gold' ||
      borderStyle === 'goldRing' ||
      borderStyle === 'silver' ||
      borderStyle === 'silverRing' ||
      borderStyle === 'rainbow';

    if (showBulbs && !bulbsDisabledForStyle) {
  ...
  const drawStyledBorder = (ctx: CanvasRenderingContext2D, centerX: number, centerY: number, radius: number, borderStyleName: string, animationTime: number, customWidth?: number) => {
    ...
    // Gestion spéciale pour le style Argent avec effets métalliques
      ...
      // Pas de trait néon supplémentaire pour l'argent afin d'éviter la surbrillance
    } 
    // Gestion des styles 'pattern' sans chargement d'images: rendu statique immédiat
      ...
      switch (borderStyleConfig.type) {
        ...
        case 'luxury':
          ...
          // Effet de glow désactivé pour les styles premium or/argent
          if (borderStyleConfig.effects.glow &&
              borderStyleName !== 'gold' &&
              borderStyleName !== 'goldRing' &&
              borderStyleName !== 'silver' &&
              borderStyleName !== 'silverRing') {
        ...
        case 'gradient':
          ...
          // Pas de glow néon pour le style arc-en-ciel afin d'éviter le trait lumineux autour
          if (borderStyleConfig.effects.glow && borderStyleName !== 'rainbow') {
